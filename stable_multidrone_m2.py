...
def main():
    ...
    writers = [
        cv2.VideoWriter(f'{output_prefix}/{name}.mp4', fourcc, effective_fps, (width, height))
    ...
    save_gps_dict_to_csv(aquas_ekalman_positions_weights, f"{output_prefix}/weighted.csv")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{output_prefix}/weighted_plot.png")
    print(f"Saved plot to {output_prefix}/weighted_plot.png")
